...
def create_collections_if_not_exists(client: qdrant_client.QdrantClient):
    ...
    if "nyayantar_legal" not in legal_names:
        ...
        logger.info("✅ Created nyayantar_legal collection")
    else:
        logger.info("✅ nyayantar_legal collection already exists")
    ...
    if "nyayantar_general" not in legal_names:
        ...
        logger.info("✅ Created nyayantar_general collection")
    else:
        logger.info("✅ nyayantar_general collection already exists")
...
def index_legal_collection(client: qdrant_client.QdrantClient, documents: List[Dict[str, Any]]):
    ...
    logger.info("🔄 Indexing documents into nyayantar_legal collection...")
    ...
    try:
        ...
        logger.info("✅ Legal embedding model loaded (using BGE-large)")
        ...
        logger.info("✅ Completed indexing into nyayantar_legal collection")
        
    except Exception as e:
        logger.error(f"❌ Error indexing legal collection: {e}")
        raise
...
def index_general_collection(client: qdrant_client.QdrantClient, documents: List[Dict[str, Any]]):
    ...
    logger.info("🔄 Indexing documents into nyayantar_general collection...")
    ...
    try:
        ...
        if not openai_api_key:
            logger.warning("⚠️ OPENAI_API_KEY not found, using FastEmbed fallback")
            from llama_index.embeddings.fastembed import FastEmbedEmbedding
        ...
        logger.info("✅ Completed indexing into nyayantar_general collection")
        
    except Exception as e:
        logger.error(f"❌ Error indexing general collection: {e}")
        raise


def main():
    """Main indexing function"""
    logger.info("🚀 Starting hybrid indexing process...")
    ...
    logger.info("ℹ️  Skipping nyayantar_legal - already populated with InLegalBERT")
