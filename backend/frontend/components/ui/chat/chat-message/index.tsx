...
function ChatMessageContent({
  ...
  const eventDataRaw = getAnnotationData<EventData | EventData[]>(
    annotations,
    MessageAnnotationType.EVENTS
  );
  // Support both single event (data = {title}) and multiple (data = [{title}, ...])
  const eventData = eventDataRaw.flatMap((d) => (Array.isArray(d) ? d : [d]));
  ...
  const contents: ContentDisplayConfig[] = [
    ...
    },
    {
      ...
      component:
        eventData.length > 0 ? (
          <ChatEvents isLoading={isLoading} data={eventData as EventData[]} />
        ) : null,
    },
    {
      order: 2,
