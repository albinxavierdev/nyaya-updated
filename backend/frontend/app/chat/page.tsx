...
function ChatContent() {
  ...
  const hasAutoSubmittedQuery = useRef(false);
  ...
  const handleNewChat = useCallback(async (preserveQuery?: string) => {
    ...
    try {
      ...
      const queryPart = preserveQuery
        ? `&query=${encodeURIComponent(preserveQuery)}`
        : '';
      router.push(`/chat?conversation_id=${newConversationId}${queryPart}`);
    } catch (error: any) {
  ...
  const {
    ...
    api: (() => {
      const base = backend || process.env.NEXT_PUBLIC_SERVER_URL || 'http://localhost:8000';
      if (!isAuthenticated) return `${base}/api/chat/guest`;
      // Always use backend URL; never empty string (would request current origin and get HTML)
      const q = conversationId ? `?conversation_id=${conversationId}` : '';
      return `${base}/api/chat${q}`;
    })(),
    headers: {
  ...
  useEffect(() => {
    if (isClient) {
      ...
      if (query && !hasAutoSubmittedQuery.current) {
        setInput(query);
      }

      if (!urlConversationId) {
        const timer = setTimeout(() => {
          handleNewChat(query ?? undefined);
        }, 1000);
  ...
  // Auto-submit query when we landed from main page with ?query=...
  useEffect(() => {
    if (!isClient || hasAutoSubmittedQuery.current) return;
    const query = searchParams.get('query');
    ...
    hasAutoSubmittedQuery.current = true;
    ...
    if (conversationId && conversationId !== 'guest') {
      router.replace(`${pathname}?conversation_id=${conversationId}`);
    }
  }, [isClient, conversationId, searchParams, append, setInput, router, pathname, isAuthenticated, queryLimitContext.canMakeQuery]);
